...
?>
    ...
    <body>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $del_query = "DELETE FROM tblstudents WHERE StudentID='$std_id'";
            if (mysqli_query($connection, $del_query)) {
                $del_query2 = "DELETE FROM tblresult WHERE fk_studentID='$std_id'";
                if (mysqli_query($connection, $del_query2)) {
                    echo "Deleted Record";
                }
            } else {
                echo "Deleted Record from students table";
            }
        } else {
